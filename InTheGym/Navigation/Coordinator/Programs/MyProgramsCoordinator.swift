...
extension MyProgramsCoordinator {
    ...
    func showSavedProgram(_ program: SavedProgramModel, listener: CurrentValueSubject<CurrentProgramModel?,Never>) {
         let child = SavedProgramDisplayCoordinator(navigationController: navigationController, program: program, listener: listener)
        childCoordinators.append(child)
        child.start()
    }
}
