...
// MARK: - Configure
private extension BoxBreathingView {
    func setupUI() {
...
extension BoxBreathingView {
    public func setDisplay(to stage: BreathingStage) {
        ...
        case .breatheIn:
            centerLabel.text = "In"
            UIView.animate(withDuration: 5, delay: 0.0, options: .curveLinear) {
                self.centerCircle.transform = CGAffineTransform.identity.scaledBy(x: 1.3, y: 1.3)
                self.centerLabel.transform = CGAffineTransform.identity.scaledBy(x: 1.3, y: 1.3)
            }
        case .breatheOut:
