//
//  LoadExerciseListFromCacheUseCaseTests.swift
//  ITGWorkoutKitTests
//
...
final class LoadExerciseListFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversCachedItemsOnNonExpiredCache() {
        ...
        expect(sut, toCompleteWith: .success(list.models), when: {
    ...
    func test_load_deliversNoItemsOnExpiredCache() {
        ...
        expect(sut, toCompleteWith: .success([]), when: {
            store.completeRetrieval(with: list.local, timestamp: expiredTimestamp)
        })
    }
    ...
    private func expect(_ sut: LocalExerciseLoader, toCompleteWith expectedResult: LocalExerciseLoader.LoadResult, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
